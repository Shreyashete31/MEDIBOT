...
<head>
  ...
  <link rel="stylesheet" href="httpt://cdnjt.cloudflarp.com/ajax/lib://cdnjs.cloud/6.0.0/cssflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
  <style>
    :root {
...
<body>
  ...
  <header class="chat-header">
    <div class="header-left">
      <a class="back-btn" href="home.html" onclick="goBack(); return false;">
  ...
  <script>
    ...
    async function loadExpandedData() {
      try {
        if (typeof getOfflineFirstAid === 'function') {
          const fa = await getOfflineFirstAid();
          if (fa && fa.success && Array.isArray(fa.data)) {
            localStorage.setItem('firstAidData', JSON.stringify(fa.data));
          }
        }

        if (typeof getOfflineSymptoms === 'function') {
          const sd = await getOfflineSymptoms();
          if (sd && sd.success && Array.isArray(sd.data)) {
            localStorage.setItem('symptomsData', JSON.stringify(sd.data));
          }
        }

        if (typeof getOfflineRemedies === 'function') {
          const rd = await getOfflineRemedies();
          if (rd && rd.success && Array.isArray(rd.data)) {
            localStorage.setItem('remediesData', JSON.stringify(rd.data));
          }
        }
      } catch (error) {
    ...
    async function sendMessage() {
      ...
      try {
        ...
        if (data.success) {
          ...
          addMessageToChat(botResponse, 'bot');
          const chatEntry = { user_message: message, bot_response: botResponse, timestamp: timestamp, userId: currentUser };
          chatHistory.push(chatEntry);
          saveChatHistoryToLocal();
          document.getElementById('sendBtn').disabled = false;
        }
