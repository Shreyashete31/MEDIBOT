...
<head>
  ...
  <style>
    ...
    * {
    ...
    .suggestion-chip:hover { background: #f5fafe; }
  ...
  <script>
    ...
    async function initializeFirstAid() {
      try {
        ...
        if (data.success) {
          allFirstAid = data.data.map(item => ({
            ...
            keywords: Array.isArray(item.keywords) ? item.keywords : generateKeywords(item)
          })).map(normalizeFirstAidItem);
      ...
      try {
        ...
        allFirstAid = mergeFirstAid(Array.isArray(baseData) ? baseData : (baseData.items || baseData), []);
        allFirstAid = allFirstAid.map(normalizeFirstAidItem);
      } catch (err) {
        // Final fallback to embedded sample if assets unavailable
        allFirstAid = firstAidData.map(normalizeFirstAidItem);
      }
    ...
    function renderFirstAid(instructions) {
      ...
      const readSet = new Set(JSON.parse(localStorage.getItem('readFirstAid') || '[]'));
      grid.innerHTML = instructions.map(instruction => `
        <div class="first-aid-card ${instruction.emergency ? 'emergency' : ''}">
          <div class="card-header">
            ...
            <h3 class="card-title">
              ${instruction.emergency ? '🚨' : '🩹'} ${instruction.title}
            </h3>
            <p class="card-description">${instruction.description || 'Follow the steps below carefully.'}</p>
          </div>
          ...
          <div class="card-actions">
            ${readSet.has(instruction.id) 
              ? `<button class="action-btn completed" id="mark-${instruction.id}" disabled>
                   <i class="fas fa-check"></i>
                   Read
                 </button>`
              : `<button class="action-btn ${instruction.emergency ? 'emergency' : ''}" id="mark-${instruction.id}" onclick="markAsRead('${instruction.id}')">
                   <i class="fas fa-check"></i>
                   Mark as Read
                 </button>`}
    ...
    function normalizeFirstAidItem(item = {}) {
      const category = String(item.category || 'general').toLowerCase();
      const description = item.description || `${String(item.title || 'First Aid').trim()} guidance`;
      const keywords = Array.isArray(item.keywords) ? item.keywords : generateKeywords(item);
      return {
        ...item,
        category,
        description,
    ...
    function initializeSearch() {
      ...
      searchInput.addEventListener('input', function() {
        ...
        filteredFirstAid = allFirstAid.filter(instruction => {
          const hasKeywords = Array.isArray(instruction.keywords) && instruction.keywords.some(k => String(k).toLowerCase().includes(query));
        ...
        if (filteredFirstAid.length === 0 && query.length > 0) {
          const fallback = getKeywordGuide(query);
          if (fallback) filteredFirstAid = [fallback];
        }
        
        renderFirstAid(filteredFirstAid);
        updateCategoryStatus(filteredFirstAid.length);
      });
    }
    ...
    function renderSuggestions(query) {
      ...
      for (let i = 0; i < allFirstAid.length; i++) {
        const it = allFirstAid[i];
        const t = String(it.title || '').toLowerCase();
        const d = String(it.description || '').toLowerCase();
        const c = String(it.category || '').toLowerCase();
        const kws = Array.isArray(it.keywords) ? it.keywords.map(k => String(k).toLowerCase()) : [];
        const match = t.includes(q) || d.includes(q) || c.includes(q) || kws.some(k => k.includes(q));
        if (match) {
          const lab = it.title || it.category || '';
          const key = lab.toLowerCase();
          if (lab && !seen.has(key)) { seen.add(key); items.push(lab); }
        }
        if (items.length >= 8) break;
      }
      const base = ['Burn','Cut','Bleeding','Choking','Sprain','Fever','Nosebleed','Poisoning'];
      for (let j = 0; j < base.length && items.length < 8; j++) {
        const b = base[j];
        if (b.toLowerCase().includes(q)) items.push(b);
      }
      el.innerHTML = items.slice(0,8).map(l => '<span class="suggestion-chip" onclick="applySearch(\'' + String(l).replace(/'/g, "\\'") + '\')">' + l + '</span>').join('');
    }

    function applySearch(label) {
    ...
    function getKeywordGuide(query) {
      const q = query.toLowerCase();
      const match = (arr) => arr.some(k => q.includes(k));
      const base = (id, title, category, desc, steps, warnings, severity) => ({
        id,
        title,
        category,
        emergency: ['emergency','respiratory','poisoning'].includes(category),
        description: desc,
        steps,
        warnings,
        severity
      ...
      if (match(['burn','scald'])) {
        return base(
          ...
          'wounds',
          'Immediate steps for minor thermal burns.',
          [
            'Cool the burn under cool running water for 10–15 minutes',
            'Remove rings or tight items near the area',
            'Cover with sterile, non-adhesive dressing',
            'Avoid ice, butter, or ointments',
            'Seek care if the burn is large or on face/hands/genitals'
          ],
          'Do not pop blisters. For severe or electrical burns, call emergency services.',
          'medium'
        );
      }

      if (match(['cut','laceration','bleed','bleeding'])) {
        return base(
          'kw-cut',
          'Bleeding Control (Minor Cuts)',
          'wounds',
          'First aid steps to stop bleeding from minor cuts.',
          [
            'Apply direct pressure with clean cloth for 5–10 minutes',
            'Rinse with clean water when bleeding slows',
            'Apply sterile dressing and keep clean/dry',
            'Elevate injured area if possible',
            'Seek care for deep or contaminated wounds'
          ],
          'Do not remove deeply embedded objects; seek professional care.',
          'medium'
        );
      }

      if (match(['choke','choking'])) {
        return base(
          'kw-choking',
          'Choking (Adult) First Aid',
          'respiratory',
          'Steps to assist a conscious adult who is choking.',
          [
            'Ask "Are you choking?" and stand behind the person',
            ...
            'Seek medical evaluation after resolution'
          ],
          'Do not perform blind finger sweeps. Call emergency services if severe.',
          'high'
        );
      }

      if (match(['sprain','strain'])) {
        return base(
          'kw-sprain',
          'Sprain Care (R.I.C.E.)',
          'wounds',
          'Supportive care for mild sprains.',
          [
            'Rest the injured area',
            'Ice 15–20 minutes every 2–3 hours',
            'Compress with elastic bandage',
            'Elevate above heart level',
            'Avoid weight-bearing if painful; seek care if severe'
          ],
          'Do not apply ice directly to skin; wrap ice pack.',
          'low'
        );
      }
      ...
      if (match(['fever'])) {
        return base(
          ...
          'Fever Self-Care',
          'medical',
          'Guidance for mild fever management.',
          [
            'Hydrate frequently',
            'Rest in a cool environment',
            'Use light clothing and cool compresses',
            'Consider antipyretics per label instructions',
            'Seek medical care for high or persistent fever'
          ],
          'High fever, severe headache, stiff neck, or breathing difficulty require urgent evaluation.',
          'medium'
        );
      }

      if (match(['nosebleed','nose bleed'])) {
        return base(
          'kw-nosebleed',
          'Nosebleed First Aid',
          'wounds',
          ...
          [
            ...
            'Pinch soft part of nose for 10–20 minutes',
            'Apply a cold compress to bridge of nose',
            'Avoid blowing nose for several hours',
            'Seek care if bleeding persists'
          ],
          'Do not tilt head back; blood may go down the throat.',
          'low'
        );
      }

      if (match(['poison','overdose'])) {
        return base(
          'kw-poison',
          'Poisoning (Ingestion) First Aid',
          'poisoning',
          'Immediate steps for suspected ingestion of a harmful substance.',
          [
            'Call emergency services or poison control immediately',
            'Do not induce vomiting unless instructed',
            'Check responsiveness and breathing',
            'Provide substance details if known',
            'Follow professional instructions until help arrives'
          ],
          'Do not give anything by mouth unless directed by professionals.',
          'critical'
        );
      }
    ...
    function markAsRead(instructionId) {
      ...
      const btn = document.getElementById(`mark-${instructionId}`);
      if (btn) {
    ...
    function shareInstructions(instructionId) {
      ...
      const stepsText = Array.isArray(instruction.steps) ? instruction.steps.map((s, idx) => `${idx+1}. ${s}`).join('\n') : '';
      const warningsText = instruction.warnings ? `\nWarnings: ${instruction.warnings}` : '';
      const shareText = `First Aid Guide\n\nTitle: ${instruction.title}\nDescription: ${instruction.description}\n\nSteps:\n${stepsText}${warningsText}`;
      
      if (navigator.share) {
        try { navigator.share({ title: instruction.title, text: shareText }); return; } catch(e) {}
      }
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(shareText).then(() => { alert('First aid guide copied to clipboard'); }).catch(()=>fallbackCopyText(shareText));
      } else {
    ...
    function fallbackCopyText(text) {
      ...
      try { document.execCommand('copy'); alert('First aid guide copied to clipboard'); } catch(e) {}
      ta.remove();
    }
